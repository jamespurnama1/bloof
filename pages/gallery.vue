...
onMounted(() => {
  ...
  // Get canvas view
  const view = document.querySelector('.bgRenderer')
  // Loaded resources will be here
  const resources = PIXI.Assets.resources
  ...
  function initApp() {
    ...
    const stageFragmentShader = stageFragment
  ...
  function initBackground() {
    ...
    const backgroundFragmentShader = backgroundFragment
  ...
  function loadTextureForImage(index) {
    ...
    // const url = `${CMSStore.galleryData.media[index].imgix_url}?fit=crop?w=${image.width}&h=${image.height}`
  ...
  function onResize() {
    ...
    resizeTimer = setTimeout(() => {
      clean()
      if (!CMSStore.galleryData && !CMSStore.galleryData.media.length) return;
      init()
    }, 200)
  }
  ...
  // Init the app
  // watch(CMSStore.galleryData, async (new, old) => {
  //   if (!CMSStore.galleryData && !CMSStore.galleryData.media.length) return;
    init()
  // })
})
...
<template>
  <section class="md:ml-[100px] h-[100dvh] pt-24 md:pt-0">
    ...
    <!-- </div> -->
    <canvas ref="bgRenderer" class="bgRenderer absolute w-full h-full -z-0 top-0 right-0 bg-warm-200" />
  </section>
</template>
